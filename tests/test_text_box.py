...
# 1. Поле Result содержит Name (Fluent Wait)
def test_fluent_wait_result_contains_name(text_box_page):
    data = {
        "name": "Петр Петров",
    ...
    wait = WebDriverWait(
        text_box_page.driver,
        timeout=5,
        poll_frequency=0.2,
        ignored_exceptions=(NoSuchElementException, StaleElementReferenceException),
    )

    wait.until(EC.text_to_be_present_in_element(text_box_page.OUTPUT, data["name"]))

    assert data["name"] in text_box_page.get_result_text()


# 2. Все поля формы заполнены кириллицей:
...
# 3. Все поля формы заполнены латиницей:
...
# 4. В поле Full Name указано полное ФИО (с отчеством):
...
# 5. В поле Full Name указано короткое значение:
...
# 6. В поле Full Name указано длинное значение:
...
# 7. В поле Full Name присутствует дефис:
...
# 8. В поле Email буквы разных регистров:
...
# 9. В поле Email присутствуют цифры:
...
# 10. Значения полей current_address и permanent_address совпадают:
...
# 11. В поле current_address указан адрес с городом, улицей, номерами дома и квартиры:
...
# 12. В поле current_address указан адрес с городом, улицей, номерами дома, строения и квартиры:
...
# 13. В поле current_address указан адрес с городом и улицей:
...
# 14. В поле current_address указан длинный адрес:
...
# 15. В поле current_address указан короткий адрес:
...
# 16. В поле current_address присутствуют спецсимволы:
...
# 17. В поле permanent_address указан адрес с городом, улицей, номерами дома и квартиры:
...
# 18. В поле permanent_address указан адрес с городом, улицей, номерами дома, строения и квартиры:
...
# 19. В поле permanent_address указан адрес с городом и улицей:
...
# 20. В поле permanent_address указан длинный адрес:
...
# 21. В поле permanent_address указан короткий адрес:
...
# 22. В поле permanent_address присутствуют спецсимволы:
...
# 23. Форма отправляется, если все поля формы пустые (позитивная проверка, так как все поля формы необязательные):
